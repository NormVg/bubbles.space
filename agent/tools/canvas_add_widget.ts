...
export default defineTool({
  ...
  inputSchema: z.object({
    type: z.enum(['markdown', 'mermaid', 'timer', 'graph', 'image', 'pdf']).describe("The type of widget to create."),
    ...
    data: z.record(z.any()).describe("The content of the widget. For markdown: { content: '...' }. For mermaid: { chart: '...' }. For timer: { duration: 300 } (duration is in seconds). For graph: { graphType: 'line' | 'bar' | 'pie', labels: ['Q1', 'Q2', 'Q3', 'Q4'], datasets: [{ label: 'Sales', values: [100, 200, 150, 300] }] }. For image: { images: ['url1', 'url2'] }. For pdf: { url: 'https://example.com/document.pdf' }")
  }),
  async execute(input) {
