...
const toggleVoice = async () => {
  if (voiceAgent.isListening.value) {
    ...
    const spokenText = voiceAgent.transcript.value
    if (spokenText && !voiceAgent.isAutoSendMode.value) {
      text.value = baseText ? baseText + (baseText.endsWith(' ') ? '' : ' ') + spokenText : spokenText
    }
  } else {
    baseText = text.value
...
watch(() => voiceAgent.transcript.value, (newVal) => {
  if ((voiceAgent.isListening.value || voiceAgent.isProcessingVoice.value) && !voiceAgent.isAutoSendMode.value) {
